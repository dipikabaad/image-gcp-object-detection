...
sys.path.append('/opt/models/slim')
...
for image_output in final_output:
	existing_boxes = {}
    	for i in range(len(image_output['detection_boxes'])):
        	...
        	if (image_output['detection_scores'][i] > 0.5) or (category_index[image_output['detection_classes'][i]]['name'] in ['horse', 'wine glass']):
            		# [class, score]
            		...
            		if not(horse_with_less_prob):
                		existing_boxes[key_dict] = [ category_index[image_output['detection_classes'][i]]['name'], image_output['detection_scores'][i] ]
	j += 2
...
df2.to_csv('/data/Netlight_Object_Detection/netlight_object_detection.csv')
df3 = pd.DataFrame.from_dict(img_by_objs)
df3.to_csv('/data/Netlight_Object_Detection/netlight_objects_identified.csv')
print(df3)
